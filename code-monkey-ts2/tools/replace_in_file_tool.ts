...
export class ReplaceInFileTool extends IOTool {
    ...
    description = "Replace all occurrences of given text in a file with the replacement";
    schema = schema;

    @instrument("Tool._call", { tool: "ReplaceInFileTool" })
    async _call({ fname, oldText, newText }: z.infer<typeof schema>): Promise<string> {
