...
export type ChatModelConstructor = (modelName: string, extraFlags: Record<string, string>) => BaseChatModel;

function constructAnthropic(modelName: string, extra_flags: Record<string, string>): BaseChatModel {
...
function constructOpenai(modelName: string, extra_flags: Record<string, string>): BaseChatModel {
    return new ChatOpenAI({ model: modelName, modelKwargs: extra_flags });
}

function constructOllama(modelName: string, extra_flags: Record<string, string>): BaseChatModel {
    // return new ChatOllama(modelName);
    throw new Error("Not implemented");
}

function constructFireworks(modelName: string, extra_flags: Record<string, string>): BaseChatModel {
...
export function get_model_service_ctor(modelService: string): ChatModelConstructor {
